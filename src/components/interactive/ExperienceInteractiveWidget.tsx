...
export default function ExperienceInteractiveWidget({
  ...
  const labels = {
    es: {
      ...
      hqPackPartners: [
        ...
        { id: 'TUV', name: 'TÜV Rheinland', tag: 'Auditoría y Certificación QMS' },
        { id: 'ISO14644', name: 'ISO 14644', tag: 'Estándar de Sala Limpia Cl. 5' },
        { id: 'ISAH', name: 'ERP ISAH', tag: 'Gobernanza y Trazabilidad' },
    ...
    en: {
      ...
      validatedTier1: 'Validated Automotive Tier-1 Supply Ecosystem:',
      hqPackPartners: [
        { id: 'ASML', name: 'ASML', tag: 'Lithography EUV/DUV' },
        { id: 'ZEISS', name: 'Zeiss', tag: 'High-NA Optics' },
        { id: 'TUV', name: 'TÜV Rheinland', tag: 'Audit & QMS Certification' },
        { id: 'ISO14644', name: 'ISO 14644', tag: 'Cleanroom Particulate Standard' },
        { id: 'ISAH', name: 'ERP ISAH', tag: 'Governance & Work Orders' },
    ...
    pt: {
      ...
      hqPackPartners: [
        ...
        { id: 'ZEISS', name: 'Zeiss', tag: 'Óptica High-NA' },
        { id: 'TUV', name: 'TÜV Rheinland', tag: 'Auditoria & Certificação QMS' },
        { id: 'ISO14644', name: 'ISO 14644', tag: 'Norma de Sala Limpa Cl. 5' },
        { id: 'ISAH', name: 'ERP ISAH', tag: 'Governança & Rastreabilidade' },
    ...
    nl: {
      ...
      hqPackPartners: [
        ...
        { id: 'TUV', name: 'TÜV Rheinland', tag: 'Audit & QMS Certificering' },
        { id: 'ISO14644', name: 'ISO 14644', tag: 'Cleanroom Standaard Kl. 5' },
        { id: 'ISAH', name: 'ERP ISAH', tag: 'Governance & Traceerbaarheid' },
    ...
    de: {
      ...
      hqPackPartners: [
        ...
        { id: 'TUV', name: 'TÜV Rheinland', tag: 'Audit & QMS Zertifizierung' },
        { id: 'ISO14644', name: 'ISO 14644', tag: 'Reinraum-Standard Kl. 5' },
        { id: 'ISAH', name: 'ERP ISAH', tag: 'Governance & Rückverfolgbarkeit' },
    ...
    fr: {
      ...
      hqPackPartners: [
        ...
        { id: 'TUV', name: 'TÜV Rheinland', tag: 'Audit & Certification QMS' },
        { id: 'ISO14644', name: 'ISO 14644', tag: 'Norme Salle Blanche Cl. 5' },
        { id: 'ISAH', name: 'ERP ISAH', tag: 'Gouvernance & Traçabilité' },
  ...
  // =========================================================================
  // 1. HQ PACK: Cleanroom with ASML, Zeiss, Frencken, Neways, etc.
  // =========================================================================
  if (experienceId === 'hq-pack') {
    ...
    return (
      <div className="p-3.5 sm:p-5 rounded-2xl bg-slate-950 dark:bg-slate-950 light:bg-white border border-cyan-500/40 dark:border-cyan-500/40 light:border-cyan-400 space-y-3.5 relative overflow-hidden shadow-lg">
        {/* Header Bar */}
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 dark:border-slate-800 light:border-slate-200 pb-2.5">
          <div className="flex items-center space-x-2">
            <span className="w-2 h-2 rounded-full bg-cyan-400 animate-ping" />
            <span className="text-[11px] sm:text-xs font-mono font-bold text-cyan-400 dark:text-cyan-300 light:text-cyan-700 tracking-wider">
              {t.cleanroomHeader}
            </span>
          </div>
          <div className="flex items-center space-x-2.5 text-[10px] sm:text-[11px] font-mono text-slate-400 light:text-slate-600">
            <strong className="text-emerald-400">{t.erpSync}</strong>
            <span>&bull;</span>
            <strong className="text-cyan-400">{t.cycleTimeRed}</strong>
          </div>
        </div>

        {/* Interactive Partner Logo Grid - Solid Opaque & Proportional */}
        <div className="space-y-1.5">
          <div className="text-[10px] font-mono text-slate-400 light:text-slate-600">
            {t.inspectPartner}
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {partners.map((p) => (
              <button
                ...
                className={`p-2 sm:p-2.5 rounded-lg border text-left transition-all duration-200 flex flex-col justify-between ${
                  activePartner === p.id
                    ? 'bg-slate-900 dark:bg-slate-900 light:bg-cyan-50 border-cyan-400 shadow-md ring-1 ring-cyan-400/50 scale-[1.02]'
                    : 'bg-slate-900 dark:bg-slate-900 light:bg-white border-slate-800 dark:border-slate-800 light:border-slate-200 hover:border-cyan-500/40 hover:bg-slate-850'
                }`}
              >
                <div className="h-6 flex items-center">
                  <BrandLogo name={p.name} className="h-5 w-auto max-w-[80px]" />
                </div>
                <div className="text-[9px] font-mono text-slate-400 light:text-slate-600 mt-1.5 truncate">
                  {p.tag}
                </div>
              </button>
            ))}
        ...
        <div className="p-2.5 sm:p-3 rounded-xl bg-slate-900 dark:bg-slate-900 light:bg-slate-100 border border-slate-800 dark:border-slate-800 light:border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 text-[11px]">
          <div className="flex items-center space-x-2">
            <Cpu className="w-4 h-4 text-cyan-400 shrink-0" />
            <div>
              <span className="font-mono font-bold text-slate-300 light:text-slate-700">{t.activeStandard} </span>
              <span className="text-cyan-400 dark:text-cyan-300 font-bold">{activePartner} Precision Cleanroom Interface</span>
            </div>
          </div>
  ...
  if (experienceId === 'vdl-nedcar') {
    ...
    return (
      <div className="p-3.5 sm:p-5 rounded-2xl bg-slate-950 dark:bg-slate-950 light:bg-white border border-cyan-500/40 dark:border-cyan-500/40 light:border-cyan-400 space-y-3.5 shadow-lg">
        {/* Header */}
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 dark:border-slate-800 light:border-slate-200 pb-2.5">
          <div className="flex items-center space-x-2">
            <Car className="w-3.5 h-3.5 text-emerald-400 animate-pulse" />
            <span className="text-[11px] sm:text-xs font-mono font-bold text-emerald-400 dark:text-emerald-300 light:text-emerald-700 tracking-wider">
              {t.automotiveHeader}
            </span>
          </div>
          <div className="text-[10px] sm:text-[11px] font-mono text-cyan-400 font-bold">
            {t.throughput}
          </div>
        </div>

        {/* Automotive OEM Brand Logos Strip - Solid Opaque & Proportional */}
        <div className="space-y-1.5">
          <div className="text-[10px] font-mono text-slate-400 light:text-slate-600">
            ...
            {autoPartners.map((p) => (
              <button
                ...
                className={`p-2 rounded-lg border text-left transition-all flex flex-col justify-between ${
                  activeAuto === p.id
                    ? 'bg-slate-900 dark:bg-slate-900 light:bg-cyan-50 border-cyan-400 shadow-md ring-1 ring-cyan-400/50 scale-[1.02]'
                    : 'bg-slate-900 dark:bg-slate-900 light:bg-white border-slate-800 dark:border-slate-800 light:border-slate-200 hover:border-cyan-500/40'
                }`}
              >
                <div className="h-5 flex items-center">
                  <BrandLogo name={p.name} className="h-4.5 w-auto max-w-[75px]" />
                </div>
                <div className="text-[8.5px] font-mono text-slate-400 light:text-slate-600 mt-1 truncate">
                  {p.tag}
                </div>
              </button>
            ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
          {stations.map((st) => (
            <div
              key={st.id}
              className="p-2.5 rounded-lg bg-slate-900 dark:bg-slate-900 light:bg-white border border-slate-800 dark:border-slate-800 light:border-slate-200"
            >
              <div className="flex items-center justify-between">
                <span className="text-[9px] font-mono text-slate-500">GATE 0{st.id}</span>
                <span className="text-[9px] font-mono text-emerald-400 font-bold">{st.status}</span>
              </div>
              <div className="text-[11px] font-bold text-slate-100 light:text-slate-900 mt-0.5">{st.name}</div>
              <div className="text-[9px] font-mono text-cyan-400 mt-0.5">{st.spec}</div>
            </div>
          ))}
        </div>

        {/* Telemetry Bar */}
        <div className="p-2.5 rounded-lg bg-slate-900 dark:bg-slate-900 light:bg-slate-100 border border-slate-800 dark:border-slate-800 light:border-slate-200 text-[11px] font-mono text-slate-300 light:text-slate-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
          <span>{t.rcaProtocol}</span>
          <span className="text-cyan-400 font-semibold">{t.multiPlatform}</span>
        </div>
      </div>
    );
  }
  ...
  if (experienceId === 'arkcohogar') {
    return (
      <div className="p-3.5 sm:p-5 rounded-2xl bg-slate-950 dark:bg-slate-950 light:bg-white border border-cyan-500/40 dark:border-cyan-500/40 light:border-cyan-400 space-y-2.5 text-xs shadow-lg">
        <div className="flex items-center justify-between border-b border-slate-800 dark:border-slate-800 light:border-slate-200 pb-2 font-mono">
          <span className="font-bold text-cyan-400 text-[11px]">{t.stockAccuracy.replace(/[0-9.%]*/, '').trim() || 'INVENTORY CONTROL & LOGISTICS AUDITING'}</span>
          <span className="text-emerald-400 font-bold text-[11px]">{t.stockAccuracy}</span>
        </div>
        <div className="grid grid-cols-3 gap-2 text-center">
          <div className="p-2 rounded-lg bg-slate-900 dark:bg-slate-900 light:bg-white border border-slate-800 dark:border-slate-800 light:border-slate-200 font-mono">
            <div className="text-cyan-400 font-bold text-[11px]">{t.arkcohogarStats[0].title}</div>
            <div className="text-[9px] text-slate-400">{t.arkcohogarStats[0].desc}</div>
          </div>
          <div className="p-2 rounded-lg bg-slate-900 dark:bg-slate-900 light:bg-white border border-slate-800 dark:border-slate-800 light:border-slate-200 font-mono">
            <div className="text-emerald-400 font-bold text-[11px]">{t.arkcohogarStats[1].title}</div>
            <div className="text-[9px] text-slate-400">{t.arkcohogarStats[1].desc}</div>
          </div>
          <div className="p-2 rounded-lg bg-slate-900 dark:bg-slate-900 light:bg-white border border-slate-800 dark:border-slate-800 light:border-slate-200 font-mono">
            <div className="text-indigo-400 font-bold text-[11px]">{t.arkcohogarStats[2].title}</div>
            <div className="text-[9px] text-slate-400">{t.arkcohogarStats[2].desc}</div>
          </div>
        </div>
      </div>
    );
